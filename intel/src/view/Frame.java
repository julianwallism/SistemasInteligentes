...
public class Frame extends JFrame {
    ...
    private ArrayDeque<Boolean>[] persons;
    ...
    public Frame() {
        ...
        persons = new ArrayDeque[Elevator.N_FLOORS];
        for (int i = 0; i < persons.length; i++) {
            persons[i] = new ArrayDeque<>();
        }
        initComponents();
    }
    ...
    public class Panel extends JPanel {
        ...
        private void drawPersons(Graphics2D g) {
            for (int i = 0; i < persons.length; i++) {
                Iterator<Boolean> it = persons[i].iterator();
                int j = 0;
                while(it.hasNext()) {
                    g.drawImage(it.next() ? redperson : bluePerson, ELEVATOR_WIDTH * (j + 1), PANEL_HEIGHT - (ELEVATOR_HEIGHT * (i + 1)), ELEVATOR_HEIGHT, ELEVATOR_HEIGHT, null);
                    j++;
                }
            }
        }
